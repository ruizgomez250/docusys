...
@section('content_header')
    <div class="row">
        <div class="col-6">
            <h1 class="m-0 custom-heading">Usuarios de cada oficina</h1>
        </div>
        <div class="col-6">
            <a href="{{ route('usuariodestino.create') }}" class="btn btn-primary" style="float: right;">Asignar Usuario</a>
        </div>
    </div>
@stop
            ...
            <div class="card">
                <div class="card-body">
                    <x-adminlte-datatable id="table1" :heads="$heads" head-theme="dark" theme="light" striped hoverable with-buttons>
                        @foreach ($usuariodestino as $row)
                            <tr>
                                <td>{{ $row->id }}</td>
                                <td>{{ $row->user->name}}</td>
                                <td>{{ $row->destino->nombre }}</td>
                                
                                <td style="float:right;">
                                    <a href="{{ route('usuariodestino.edit', $row->id) }}" class="btn btn-outline-secondary">
                                        <i class="fa fa-sm fa-fw fa-pen"></i>
                                    </a>
                                    <form action="{{ route('usuariodestino.destroy', $row->id) }}" method="post" class="d-inline delete-form">
                                        @csrf
                                        @method('DELETE')
